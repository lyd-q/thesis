...
use "`path'/Data/Working/nih_raw.dta", clear
...
* full dataset of individual grants in US
use "`path'/Data/nih_US.dta", clear
rename STATEORCOUNTRYNAME state_fullname
merge m:1 state_fullname using "`path'/Data/Crosswalks/state_abbrev.dta"
sort _m state_fullname
...
save "`path'/Data/Working/nih_counties.dta", replace
//num observations: 1,729,538 out of 1,783,325 (97% match rate)
...
************************************************************
	*** DROP 2025 -- dropped year == 2025 for us, county, and cbsa file
************************************************************
	
***********
* Check if 2009 county sort crosswalk is same as CBSA downloaded version -- pretty close, differences are non-metro
***********
clear
import excel "`path'/Data/Crosswalks/county_cbsa_xwalk_2009.xlsx", firstrow
save "`path'/Data/Crosswalks/county_cbsa_2009.dta", replace

import delimited "`path'/Data/Crosswalks/county_2009_sort.csv", clear
drop v1
rename cnty county_fips
tostring county_fips, replace
merge 1:m county_fips using "`path'/Data/Crosswalks/county_cbsa_2009.dta", force
